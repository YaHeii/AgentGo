...
func TestSendMessageCreatesPlaceholderAndStreamsToCompletion(t *testing.T) {
	t.Parallel()
	...
	require.Equal(t, store.MessageStatusComplete, result.Assistant.Status)
	require.Equal(t, "hello", result.Assistant.Content)
	require.Len(t, st.createdMessages, 2)
}

func TestSendMessageMarksAssistantFailedWhenStreamErrors(t *testing.T) {
	t.Parallel()
...
func TestSendMessageUsesPriorConversationHistory(t *testing.T) {
	t.Parallel()
